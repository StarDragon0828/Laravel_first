<div class="form-group" style="width: 100%;">
    ...
    <select class="form-control" id="sessionSelect">
        ...
        @foreach ($instances as $instance)
            <option value="{{ $instance->session_name }}" {{ $instance->session_name == $responder->session ? 'selected' : '' }}>
                {{ $instance->session_name }}
            </option>
        @endforeach
    </select>
    <input type="hidden" name="session" id="selectedSession" value="{{ $responder->session }}">
</div>
...
<div class="pop-form-cont mb-3">
    ...
    <div class="form-group" style="width: 100%;">
        ...
        <input type="text" name="keyword" class="form-control" id="exampleInputEmail1" placeholder="Palavara" value="{{ $responder->keyword }}">
    </div>
    <div class="form-group">
        <label>Reposta automatica <span style="color:#007bff;">[name] ra exibir o nome do usuário</span></label>
        <textarea class="form-control" id="responseTextArea" name="response" rows="3" placeholder="Enter ...">{{ $responder->response }}</textarea>
    </div>
                    ...
                    <div class="responders preview-container">
                        ...
                        <div class="content w-100">
                            @if($responder->response_file)
                                <div id="discard-auto-file" class="d-flex justify-content-end p-2">
                                    <i class="fa fa-times" id="clearFile" style="cursor: pointer"></i>
                                </div>
                                @php
                                    $fileExtension = pathinfo($responder->response_file, PATHINFO_EXTENSION);
                                @endphp
                                @if(in_array($fileExtension, ['jpg', 'jpeg', 'png']))
                                    <!-- Display image -->
                                    <img src="{{ $responder->response_file }}" class="img-preview" style="width: 100%">
                                @elseif(in_array($fileExtension, ['pdf', 'docx', 'pptx', 'xlsx', 'csv']))
                                    <!-- Display document with icon -->
                                    <i class="far fa-file-alt mr-2"></i>
                                    {{ basename($responder->response_file) }}
...
<script>
    document.addEventListener("DOMContentLoaded", function () {
        ...
        const discardAutoFile = document.getElementById("clearFile");
        discardAutoFile?.addEventListener('click', () => {
