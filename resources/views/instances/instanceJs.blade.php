...
<script>
    function generateInstanceHTML(instance) {
        ...
        const buttonHtml = instance.status == 0 ?
            ...
            <button type="button" class="qrItem btn btn-block btn-success" style="max-height: 38px; margin-top: 0; max-width: 120px; min-width: 120px;" data-qr="${instance.qr_code}" ><i class="fas fa-qrcode" style="margin-right: 5px;"></i>QRcode</button>
        ` :
            `
...
<script>
    ...
    function createUpdateQR(instanceID) {
        url = "{{ route('create-channel-qr') }}";
        if (instanceID) {
            url = url + "/" + instanceID;
        }
        ...
        $.ajax(createQR).done(function(createQRResponse) {
            console.log(createQRResponse);

            if (createQRResponse?.refresh) {
                location.reload();
            }

            clearInterval(qrTimeout);

            if (createQRResponse.url) {
                qrImage.innerHTML = '<img src="' + createQRResponse.url +
                    '" alt="whatsapp login QR code" style="width: 100%;">';
                qrLoader.style.display = 'none';

                var seconds = parseInt(createQRResponse.time_out) + 1;
                qrTimeout = setInterval(function() {
                    seconds--;
...
<script>
    ...
    channel.bind('QR', async function(data) {
        ...
        qrStatus.innerHTML = '';
        if (qrObject.status) {
            qrStatus.insertAdjacentHTML('beforeend',
                `<p style="text-align: center">Status - ${qrObject.status}</p>`);
        }
        if (qrObject.session) {
            qrStatus.insertAdjacentHTML('beforeend',
                `<p style="text-align: center">Session - ${qrObject.session}</p>`);
        }
